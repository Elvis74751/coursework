...
summarize (trips, max(birth_year))
...
trips %>% 
  ...
  summarize (trips_by_gender = n(),
             ave_trip_time_by_gender = mean(tripduration),
...
trips %>%
  mutate(day_month_year = day(starttime)) %>%
  count(day_month_year) %>%
  arrange(desc(n)) %>%
  top_n(1)
...
average_trips_by_hour <- trips %>%
  ...
  count(hour) %>%
  mutate(average_trips_by_hour = mean(n))

# what time(s) of day tend to be peak hour(s)?

average_trips_by_hour %>%
  filter(n == max(n))
