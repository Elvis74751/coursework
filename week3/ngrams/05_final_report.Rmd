...
year_counts <- read_delim( 'year_counts.tsv', delim = '\t' , col_names = c('term','year','volume','book_count' ))
...
total_counts %>% summarize(n())

year_counts %>% summarize(n())
...
year_proportion <- inner_join(total_counts,year_counts, by=c('year'='year')) %>% mutate('proportion_of_total' = volume/total_volume, 'term_factor' = factor(term))
...
figure_3a <- year_proportion %>% filter(term == '1883' | term == '1910' | term == '1950') %>% filter(year >= 1850 & year <= 2012)

figure_3a %>% ggplot(aes(x = year, y = proportion_of_total, color = term_factor)) + geom_line() + scale_y_continuous(labels=scales::percent)
...
This graph of the data does show similar results as the graph in the paper
...
![](/Users/elvis\Downloads\Ngrams.png)

While both graphs have a similar shape, the ngram viewer has smoother curves in its graph while also having higher percentages for each of its years being graphed.
...
figure_3a_v2 <- year_proportion %>% filter(term == '1883' | term == '1910' | term == '1950') %>% filter(year >= 1850 & year <= 2012)

figure_3a_v2 %>% ggplot(aes(x = year, y = volume, color = term_factor)) + geom_line() + scale_y_continuous(labels=scales::comma)
...
total_count_1850_2012 <- total_counts %>% filter( year >= 1850 & year <= 2012)

total_count_1850_2012 %>% ggplot(aes(x = year, y = total_volume)) + geom_line() + scale_y_continuous(labels=scales::comma)
...
The difference between b and d does not lead me to reevaluate any previous results because while it is true that the raw amount of mentions of years like 1883 and 1910 are seemingly increasing as time goes on, they are being drowned out by the massive increase in overall data of other words. This is shown in the graphs that look at this problem through the perspective of percentages
...
total_count_peak <- figure_3a %>% group_by(term) %>% filter(proportion_of_total == max(proportion_of_total))
total_count_peak
...
total_count_half_life <- figure_3a %>% 
  group_by(term) %>% 
  mutate(peak_proportion = max(proportion_of_total)) %>% 
  filter(proportion_of_total <= peak_proportion/2 & year > term) %>% 
  filter(year == min(year)) %>% 
  mutate(half_life = year-term) #CHANGE TERM TO BE THE PEAK YEAR SOMEHOW 
...
total_count_half_life %>% ggplot(aes(x = year,y = half_life)) + geom_point(aes(color = term_factor)) + geom_smooth()

#gets halflife for all term years
figure_3a_all_years <- year_proportion %>% 
  filter(year >= 1850 & year <= 2012) %>%
  filter(term >= 1875 & year <= 1975) %>% 
  group_by(term) %>% 
  mutate(peak_proportion = max(proportion_of_total)) %>% 
  filter(proportion_of_total <= peak_proportion/2 & year > term) %>% 
  filter(year == min(year)) %>% 
  mutate(half_life = year-term) %>% 
  arrange(term)

#graphs all the term years
figure_3a_all_years %>% ggplot(aes(x = term,y = half_life)) + geom_point() + geom_smooth()

#shows smallest half-life

figure_3a_all_years %>% arrange(half_life) %>% 
  


#shows largest half-life

figure_3a_all_years %>% arrange(desc(half_life))
...
The data from version 2 does not produce the same outcomes as version 1 data
...
figure_3a_all_years %>% filter(half_life == max(half_life))

#shows largest half-lige

figure_3a_all_years %>% filter(half_Life == min(half_life))
