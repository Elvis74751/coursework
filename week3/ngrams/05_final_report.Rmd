...
figure_3a %>% ggplot(aes(x = year, y = proportion_of_total, color = term_factor)) + geom_line()
...
![]("C:\Users\elvis\Downloads\Ngrams.png")
...
figure_3a_v2 %>% ggplot(aes(x = year, y = volume, color = term_factor)) + geom_line()
...
total_count_1850_2012 %>% ggplot(aes(x = year, y = total_volume)) + geom_line()
...
total_count_half_life <- figure_3a %>% group_by(term) %>% mutate(peak_proportion = max(proportion_of_total)) %>% filter(proportion_of_total <= peak_proportion/2 & year > term) %>% filter(year == min(year)) %>% mutate(half_life = year-term)
